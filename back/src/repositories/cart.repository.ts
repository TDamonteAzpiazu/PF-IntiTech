...
export class CartRepository {
    ...
    async addItemToCart(cart_id: string, cart_item: CartItemDto): Promise<CartItem> {
        ...
        const cartsItems: CartItem[] = await this.cartItemRepository.find({ where: { cart } });
    ...
    async deleteItemFromCart(cart_itemId: string): Promise<string> {
        const cartItem: CartItem = await this.cartItemRepository.findOne({ where: { id: cart_itemId }, relations: ['cart'] });
    ...
    async clearCart(cart_id: string): Promise<string> {
        const cart: Cart = await this.cartRepository.findOneBy({ id: cart_id });
        const items: CartItem[] = await this.cartItemRepository.find({ where: { cart } });
    ...
    async createRecord(cart_id: string): Promise<string> {
        const cart: Cart = await this.cartRepository.findOne({ where: { id: cart_id }, relations: ['user'] });
        ...
        const record: Record = this.recordRepository.create({ totalPrice: cart.totalPrice, user: cart.user });
    ...
    async getAllRecords(user_id: string): Promise<AllRecordDto[]> {
        ...
        const records: Record[] = await this.recordRepository.find({ where: { user: { id: user_id } } });
        ...
        for (const record of records) {
            const items: ItemRecord[] = await this.itemRecordRepository.find({ where: { record } });
