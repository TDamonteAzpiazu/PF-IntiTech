...
const Profile = () => {
  ...
  const handleFileChange = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(
        `https://pf-intitech.onrender.com/files/uploadUserImage/${userData?.id}`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.localStorage) {
      const storedUserData: DataUser = JSON.parse(
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      if (
        storedUserData?.status === 'pending' &&
        !localStorage.getItem('accountActivationAlertShown')
      ) {
        Swal.fire(
          'Estado de la cuenta',
          'Debes activar tu cuenta, revisa tu correo',
          'info'
        )
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(
        `https://pf-intitech.onrender.com/users/${userData?.id}`,
        {
          method: 'PUT',
          headers: {
  ...
  return (
    <div className="h-screen pt-28 flex justify-center gap-16 mb-10">
      <form className="flex flex-col" onSubmit={handleFileChange}>
        ...
        <div className="flex flex-col p-5">
          <img
            src={newUser?.image}
            alt="imagen"
            className="flex mx-auto w-36 h-36 rounded-full"
          />
        ...
        <button
          type="submit"
          className="mx-auto flex mt-14 w-fit h-fit px-4 py-3 rounded-xl bg-lightorangeinti text-white font-medium hover:scale-105 transition-all duration-300 ease-in-out"
        >
          Guardar imagen
        </button>
      </form>
      ...
      <form className="w-[700px]" onSubmit={handleSubmit}>
        <h1 className="text-3xl text-white font-medium p-3">
          Configuración de perfil
        </h1>
        <p className="text-white text-sm font-light px-3">
          Puedes cambiar tus datos personales
        </p>
        <div className="py-11">
        ...
        <button
          type="submit"
          className="mx-auto flex mt-3 w-fit h-fit px-4 py-3 rounded-xl bg-lightorangeinti text-white font-medium hover:scale-105 transition-all duration-300 ease-in-out"
        >
          Guardar cambios
        </button>
      </form>
